...
uint h32 = murmur::hash3_x86_32(char* key, usz len, seed);

uint128 h128 = murmur::hash3_x86_128(char* key, usz len, seed);

uint128 h128 = murmur::hash3_x64_128(char* key, usz len, seed);
```
...
ulong h = city::hash64(char* buf, usz len);
ulong h = city::hash64_with_seed(char* buf, usz len, ulong seed);

// CityHash128

uint128 h = city::hash128(char* s, usz len);
uint128 h = city::hash128_with_seed(char* s, usz len, uint128 seed);

// CityHashCrc (requires SSE4.2)

uint128 h = city::hash128_crc(char* s, usz len);
uint128 h = city::hash128_crc_with_seed(char* s, usz len, uint128 seed);
void h = city::hash256_crc(char* s, usz len, ulong* result);
```
